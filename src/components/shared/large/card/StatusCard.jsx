...
const StatusCard = ({ icon, type, status, from, progressIcon, data, smallerName }) => {
  const SingleData = data?.[smallerName];
  status = SingleData?.level;
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-[20px] xl:text-[24px]">{value?.level}</h3>
            </div>
          </div>
        </div>
        {progressIcon && <div className="w-[40%]">{progressIcon}</div>}
